...
def deleting_note():
    ...
    with open('notes.cvs', 'w+', encoding='utf-8') as file:
        file.seek(0)
        file.close()
    with open('notes.cvs', 'a', encoding='utf-8') as file:
        for note in input_array:
...
def find_note_by_data():
    if path.exists('notes.cvs'):
        ...
        if flag is False:
            logging.info('Ошибка ввода')
            print('В этот день заметок не создавалось!!')
    else:
        print('Что то пошло не так!!!! Попробуйте еще раз!')
